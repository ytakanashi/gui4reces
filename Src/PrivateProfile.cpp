...
bool Config::save(bool include_gui4reces_section){
	//動作モード
	...
	//実行時に出力ファイル名選択
	//gui4reces専用項目
	write(_T("Compress"),_T("ChooseOutputFileEachTime"),m_cfg.compress.choose_output_file_each_time,m_cfg.compress.choose_output_file_each_time!=m_default_cfg.compress.choose_output_file_each_time);
	...
	//gui4reces専用項目
	if(include_gui4reces_section){
		//最前面
		write(_T("Gui4reces"),_T("TopMost"),m_cfg.gui4reces.top_most,m_cfg.gui4reces.top_most!=m_default_cfg.gui4reces.top_most);
		//デフォルトプロファイル名
		write(_T("Gui4reces"),_T("DefaultProfile"),m_cfg.gui4reces.default_profile.c_str(),!m_cfg.gui4reces.default_profile.empty());
		//gui4rcesを終了する
...
bool Config::load(bool include_gui4reces_section){
...
//	getDataEx(_T("Compress"),_T("RawFileName"),&m_cfg.compress.raw_file_name);
	...
	if(include_gui4reces_section){
		//最前面表示
		getDataEx(_T("Gui4reces"),_T("TopMost"),&m_cfg.gui4reces.top_most);
		//デフォルトプロファイル名
		getStringDataEx(_T("Gui4reces"),_T("DefaultProfile"),&m_cfg.gui4reces.default_profile);
		//gui4recesを終了する
...
//別のConfigから設定を取り込む
void Config::import(const Config& src,bool include_gui4reces_section){
	tstring cfg_path(filepath());

	GUI4RECES gui4reces_orig=m_cfg.gui4reces;

	*this=src;
	if(!include_gui4reces_section){
		m_cfg.gui4reces=gui4reces_orig;
	}
